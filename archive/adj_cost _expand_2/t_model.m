...
# fs_0=exp(a_0)*alf*exp((n_0)/exp(s_0)*q_0)^(1-alf)*exp(k(-1))^alf;
# fs_1=exp(a_1)*alf*exp((n_1)/exp(s_1)*q_1)^(1-alf)*exp(k(-1))^alf;
# fs_2=exp(a_2)*alf*exp((n_2)/exp(s_2)*q_2)^(1-alf)*exp(k(-1))^alf;
# fs_0plus=exp(a_0(+1))*alf*(exp(n_0(+1))/exp(s_0(+1))*q_plus0)^(1-alf)*exp(k)^alf;

# x_10=(q_0-exp(n_0)*psi_n/exp(n_0(-1))*(exp(n_0)/exp(n_0(-1))-1));
# x_11=(q_1-exp(n_1)*psi_n/exp(n_1(-1))*(exp(n_1)/exp(n_1(-1))-1));
# x_12=(q_2-exp(n_2)*psi_n/exp(n_2(-1))*(exp(n_2)/exp(n_2(-1))-1));

# x_20plus=psi_n*(exp(n_0(+1))/exp(n_0)-1)*(exp(n_0(+1))/exp(n_0))^2;
# x_21plus=psi_n*(exp(n_1(+1))/exp(n_1)-1)*(exp(n_1(+1))/exp(n_1))^2;
# x_22plus=psi_n*(exp(n_2(+1))/exp(n_2)-1)*(exp(n_2(+1))/exp(n_2))^2;

# z_1=1-psi_k*(exp(i)/exp(k(-1))-delt);
# z_1plus=1-psi_k*exp((i(+1))/exp(k)-delt);

# z_2plus=1-delt-psi_k/2*exp((i(+1))/exp(k)-delt)^2+psi_k*exp(i(+1))/exp(k)*(exp(i(+1))/exp(k)-delt);

exp(k_0)=exp(s_0)/exp(n_0)*exp(k(-1));
exp(k_1)=exp(s_1)/exp(n_1)*exp(k(-1));
exp(k_2)=exp(s_2)/exp(n_2)*exp(k(-1));
...
exp(k)=(1-delt)*exp(k(-1))+exp(i)-psi_k/2*(exp(i)/exp(k(-1))-delt)^2*exp(k(-1));

1=exp(n_0)+exp(n_1)+exp(n_2);

1=exp(s_0)+exp(s_1)+exp(s_2);

exp(c_0)=exp(a_0)*exp(n_0)^(1-alf)*q_0^(1-alf)*exp(s_0)^alf*exp(k(-1))^alf-exp(i);
...
exp(y_0)=exp(c_0)+exp(i);

exp(p_0)=om_0/om_0*(exp(c_0)/exp(c_0))^(-1/eps);
exp(p_1)=om_1/om_0*(exp(c_1)/exp(c_0))^(-1/eps);
exp(p_2)=om_2/om_0*(exp(c_2)/exp(c_0))^(-1/eps);


exp(y)=exp(i)+exp(c_0)*exp(p_0)+exp(c_1)*exp(p_1)+exp(c_2)*exp(p_2);
exp(r)=1;

tot=(1/exp(r)*exp(y)-exp(y(-1)))/exp(y(-1));

#sc=(
#(1/exp(r)*exp(y_0)*exp(p_0)/exp(n_0)-exp(y_0(-1))*exp(p_0(-1))/exp(n_0(-1)))*exp(n_0)+
#(1/exp(r)*exp(c_1)*exp(p_1)/exp(n_1)-exp(c_1(-1))*exp(p_1(-1))/exp(n_1(-1)))*exp(n_1)+
#(1/exp(r)*exp(c_2)*exp(p_2)/exp(n_2)-exp(c_2(-1))*exp(p_2(-1))/exp(n_2(-1)))*exp(n_2))
#/exp(y(-1));

win=(
(exp(n_0)-exp(n_0(-1)))*exp(y_0(-1))*exp(p_0(-1))/exp(n_0(-1))+
(exp(n_1)-exp(n_1(-1)))*exp(c_1(-1))*exp(p_1(-1))/exp(n_1(-1))+
(exp(n_2)-exp(n_2(-1)))*exp(c_2(-1))*exp(p_2(-1))/exp(n_2(-1)))
/exp(y(-1));
